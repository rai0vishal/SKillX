    ...
    <div className="relative w-full" ref={suggestionsRef}>
      <form onSubmit={handleSubmit} className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-16 py-4 text-base bg-white border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-indigo-400 shadow-sm transition-all placeholder-gray-400"
        />
        <button
          type="submit"
          ...
          {value.length < 2 && recentSearches.length > 0 && (
            <>
              ...
              {recentSearches.map((s, i) => (
                <button key={i} onClick={() => handleSuggestionClick(s)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition text-left">
